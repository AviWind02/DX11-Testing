    ...
    public:
        ...
        bool Option(const char* text)
        {
            ...
            if (currentOption <= maxOption && OptionCount <= maxOption)
            {
                ...
                float YPostion = 25.f * OptionCount + 55.f;
                ImVec2 TextPos{ ImGui::GetCursorScreenPos().x - 4.f, YPostion };

                const ImGuiID id = window->GetID(text);
                const ImRect bb(ImVec2(TextPos.x, TextPos.y), add(&ImVec2(TextPos.x, TextPos.y), &ImVec2(400, 25)));
                pressed = ImGui::ButtonBehavior(bb, id, &Hover, &held);


                RectFilled(MiddleBoxBack, ImVec2(TextPos.x, TextPos.y), ImVec2(400, 25));
                Hover ? RectFilled(ScrollBar, ImVec2(TextPos.x, TextPos.y), ImVec2(400, 25), false) : RectFilled(ClearColour, ImVec2(TextPos.x, TextPos.y), ImVec2(400, 25), false);
                Text(text, Hover ? TextOnHover : TextOnNomral, { TextPos.x + 10.f, TextPos.y });
            }
            else if (OptionCount > (currentOption - maxOption) && OptionCount <= currentOption)
            {
                float YPostion = 25.f * (OptionCount - (currentOption - maxOption)) + 55.f;
                ImVec2 TextPos{ ImGui::GetCursorScreenPos().x - 4.f, YPostion };
        ...
        void Title(std::string text)
        {
            ...
            RectFilled(BottomEndBox, ImVec2(gcsp.x - 10.f, 0), ImVec2(400, 80));
            //Line(RGBFade(), ImVec2(0, 80), ImVec2(400, 0), 3.f);
            Text(texttitle, TextOnNomral, TextPos, NULL);
            SubTitle = text;
        }
        void End()
        {

            int OPCount = OptionCount > 14 ? (15) : OptionCount + 1;
            std::string Count = StringToChar(std::to_string(currentOption) + "/" + std::to_string(OptionCount));
            float YPostion = 25.f * OPCount + 60.f;
            ImVec2 TextPos{ ImGui::GetCursorScreenPos().x + 10.f, YPostion };
            RectFilled(BottomEndBox, ImVec2(TextPos.x - 14.f, TextPos.y - 5.f), ImVec2(400, 25));
            Text(Count, TextOnNomral, { TextPos.x,TextPos.y + 2.f }, NULL, true);
            Text("V1.0 | " + SubTitle, TextOnNomral, { TextPos.x,TextPos.y + 2.f });
            Line(RGBFade(), ImVec2(0, YPostion), ImVec2(400, 0));
